...
class StoreCounsellorLeaveDayRequest extends FormRequest
{
    ...
    {
        return [
            'start_time.required_unless' => 'Start time is required when the leave day is not full day.',
            'end_time.required_unless' => 'End time is required when the leave day is not full day.',
            'start_time.before' => 'The leave start time must be before the end time.',
            'end_time.after' => 'The leave end time must be after the start time.',
        ];
    }
        ...
        $validator->after(function (Validator $validator): void {
            ...
            if (! $counsellorProfileId) {
                $validator->errors()->add(
                    'counsellor_profile_id',
                    'A counsellor profile is required for this leave day.'
                );

                return;
            }

            $overlapExists = CounsellorLeaveDay::query()
                ->where('counsellor_profile_id', $counsellorProfileId)
                ->whereDate('leave_date', $this->input('leave_date'))
                ->when(
                    $this->leaveDayIdToIgnore(),
                    fn (Builder $query, int $id) => $query->whereKeyNot($id)
                )
                ->where(function (Builder $query): void {
                    if ($this->boolean('is_full_day', true)) {
                        $query->whereRaw('1 = 1');
            ...
            if ($overlapExists) {
                $validator->errors()->add(
                    'leave_date',
                    'This leave day overlaps with an existing leave record for the selected counsellor.'
                );
            }
        });
    }
